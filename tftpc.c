...
/* All of the following deals with command line switches */
	while ((opt = getopt(argc, argv, "dnoh:P:p:g:l:w:")) != -1) {	/* this function is handy */
		...
		case 'p':	/* put a file on the server */
			...
			if (debug) {
				printf
				    ("Client: The file name is: %s and can be read",
				     filename);
			}
			fclose(fp);
			break;
		case 'g':	/*get a file from the server */
			...
			if (debug) {
				printf
				    ("Client: The file name is: %s and it has been created",
				     filename);
			}
			fclose(fp);
			break;
		case 'w':	/* Get the window size */
			...
			if (ackfreq > MAXACKFREQ) {
				...
				printf
				    ("Client: Sorry, you have to ack sometime.\n");
				return 0;
			}
			break;
...
void tsend(char *pFilename, struct sockaddr_in server, char *pMode, int sock)
{
	...
	if (fp == NULL) {	//if the pointer is null then the file can't be opened - Bad perms OR no such file
		if (debug)
			printf
			    ("Client: sending bad file: file not found (%s)\n",
			     filename);
		return;
	} else {
		if (debug)
	...
	for (j = 0; j < RETRIES - 2; j++) {
		...
		for (i = 0; errno == EAGAIN && i <= TIMEOUT && n < 0; i++) {
			n = recvfrom(sock, recvbuf, sizeof(recvbuf),
				     MSG_DONTWAIT, (struct sockaddr *)&ack,
	...
	while (1) {		/* our break statement will escape us when we are done */
		...
		if (sendto
		    (sock, packetbuf[bcount], len, 0,
		     (struct sockaddr *)&server, sizeof(server)) != len) {
			if (debug)
				printf
				    ("Client: Mismatch in number of sent bytes\n");
			return;
		}
		if (debug) {
			ip_port(server);
...
/* this formatting code is just like the code in the main function */
				...
				if (debug)
					printf
					    ("Client: Ack(s) lost. Resending complete.\n");
...
/* The ack sending 'for' loop ends here */
		...
		if (j == RETRIES) {
			if (debug)
				printf
				    ("Client: Ack Timeout. Aborting transfer\n");
			fclose(fp);
...
void tget(char *pFilename, struct sockaddr_in server, char *pMode, int sock)
{
	...
	if (fp == NULL) {	//if the pointer is null then the file can't be opened - Bad perms 
		if (debug)
			printf
			    ("Client requested bad file: cannot open for writing (%s)\n",
			     filename);
		return;
...
/* zero the buffer before we begin */
	...
	do {
		...
		if (n != (datasize + 4)) {	/* remember if our datasize is less than a full packet this was the last packet to be received */
			...
			if (debug)
				printf
				    ("The Client has sent an ACK for packet\n");
...
/* sending error message */
					...
					len =
					    sprintf(ackbuf, "%c%c%c%c", 0x00,
						    0x04, 0x00, 0x00);
					ackbuf[2] = (count & 0xFF00) >> 8;	//fill in the count (top number first)
					ackbuf[3] = (count & 0x00FF);	//fill in the lower part of the count
					if (debug)
						printf
						    ("Sending ack # %04d (length: %d)\n",
						     count, len);
					if (((count - 1) % ackfreq) == 0) {
						...
						if (debug)
							printf
							    ("The client has sent an ACK for packet %d\n",
							     count);
					}	//check for ackfreq
					else if (count == 1) {
						if (sendto
						...
						if (debug)
							printf
							    ("The Client has sent an ACK for packet 1\n");
					}
					break;
				}	//end of else
			}
		...
		if (j == RETRIES) {
			if (debug)
				printf
				    ("Data recieve Timeout. Aborting transfer\n");
			fclose(fp);
			return;
		}
	}
