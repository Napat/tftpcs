...
int main(int argc, char **argv)
{
	...
	while ((opt = getopt(argc, argv, "dh:p:P:a:s:")) != -1) {
		switch (opt) {
	...
	if (!debug) {
		pid = fork();
		/* if pid != 0 then we are the parent */
		if (pid != 0) {
			if (pid == -1) {
	...
	while (1) {
		...
		/*the fs message */
		n = 0;
		/* This loop is required because on linux we have to acknowledge complete children with waitpid. Ugh. */
		while (errno == EAGAIN || n == 0) {
			waitpid(-1, &status, WNOHANG);
			n = recvfrom(sock, buf, BUFSIZ, MSG_DONTWAIT,
				     (struct sockaddr *)&client,
				     (socklen_t *) & client_len);
			if (n < 0 && errno != EAGAIN) {
				printf
				    ("The server could not receive from the client");
				return 0;
			}
			usleep(1000);
		}
		...
		// RRQ or WRQ. The only two really valid packets on port 69
		...
		/* case one and two are valid on port 69 or server port... no other codes are */
		switch (opcode) {
		case 1:
			if (debug) {
		...
		default:
			if (debug)
				printf
				    ("Invalid opcode detected. Ignoring packet.");
			break;
		}
	}
}
...
void tget(char *pFilename, struct sockaddr_in client, char *pMode, int tid)
{
	...
	if (fp == NULL) {	//if the pointer is null then the file can't be opened - Bad perms 
		if (debug)
			printf
			    ("Server requested bad file: cannot open for writing (%s)\n",
			     fullpath);
		len =
		    sprintf((char *)packetbuf,
			    "%c%c%c%cFile cannot be opened for writing (%s)%c",
			    0x00, 0x05, 0x00, 0x02, fullpath, 0x00);
		if (sendto(sock, packetbuf, len, 0, (struct sockaddr *)&client, sizeof(client)) != len) {	/* send the data packet */
			printf
			    ("Mismatch in number of sent bytes while trying to send error packet\n");
		}
	...
	do {
		...
		if (count == 0 || (count % ackfreq) == 0 || n != (datasize + 4)) {	/* ack the first packet, count % ackfreq will make it so we only ACK everyone ackfreq ACKs, ack the last packet */
			len =
			    sprintf(ackbuf, "%c%c%c%c", 0x00, 0x04, 0x00, 0x00);
		...
		for (j = 0; j < RETRIES; j++) {	/* this allows us to loop until we either break out by getting the correct ack OR time out because we've looped more than RETRIES times */
			...
			if (n < 0 && errno != EAGAIN) {	/* this will be true when there is an error that isn't the WOULD BLOCK error */
				...
				if (debug)
					printf
					    ("Timeout waiting for data (errno: %d == %d n: %d)\n",
					     errno, EAGAIN, n);
				//resend packet
	...
	if (debug)
		printf
		    ("fclose and sync successful. File failed to recieve properly\n");
	return;
	...
	if (debug)
		printf
		    ("fclose and sync successful. File received successfully\n");
...
void tsend(char *pFilename, struct sockaddr_in client, char *pMode, int tid)
{
	...
	if (fp == NULL) {	//if the pointer is null then the file can't be opened - Bad perms OR no such file
		if (debug)
			printf
			    ("Server requested bad file: file not found (%s)\n",
			     fullpath);
		len =
		    sprintf((char *)packetbuf[0],
			    "%c%c%c%cFile not found (%s)%c", 0x00, 0x05, 0x00,
			    0x01, fullpath, 0x00);
		if (sendto(sock, packetbuf[0], len, 0, (struct sockaddr *)&client, sizeof(client)) != len) {	/* send the data packet */
			printf
			    ("Mismatch in number of sent bytes while trying to send error packet\n");
	...
	while (1) {		/* our break statement will escape us when we are done */
		...
		if (debug)
			printf
			    ("Sending packet # %04d (length: %d file chunk: %d)\n",
			     count, len, ssize);
		...
		if ((count - 1) == 0 || ((count - 1) % ackfreq) == 0
			...
			for (j = 0; j < RETRIES; j++) {
				...
				if (n < 0 && errno != EAGAIN) {
					...
					if (debug)
						printf
						    ("Timeout waiting for ack (errno: %d n: %d)\n",
						     errno, n);
					//resend packet
					...
					if (opcode != 4 || rcount != count) {	/* ack packet should have code 4 (ack) and should be acking the packet we just sent */
						...
						if (debug)
							printf
							    ("Remote host successfully ACK'd (#%d)\n",
							     rcount);
						break;
					}
				}
				for (i = 0; i <= bcount; i++) {
					...
					if (debug)
						printf
						    ("Ack(s) lost. Resending: %d\n",
						     count - bcount + i);
				}
				if (debug)
					printf
					    ("Ack(s) lost. Resending complete.\n");
			...
			printf
			    ("Not attempting to recieve ack. Not required. count: %d\n",
			     count);
...
int isnotvaliddir(char *pPath)
{				/* this function just makes sure that the directory passed to the server is valid and adds a trailing slash if not present */
	DIR *dp;
	int len;
	dp = opendir(pPath);
	if (dp == NULL) {
